...
float Ki_ = 0.25;
float Kd_ = 0.005; // Kd_ = 0.1 ,0.01 , 0.001;
float dt_ = 0.1;
float Ka_ = 0.8 ;
float u = 0;
...
float setSPEED(float tar_vel, float cur_vel) { // rpm
  ...
  u = P_err + tar_vel  + I_err; //+ D_err ;//+ I_err  + D_err; //+ A_err ; + tar_vel 
  //u_k = u;
  ...
  /*inverse function */
  //u_c = 145.5942 - sqrt(21197 - (8083.7*u_k));
  //u_c_target = 145.5942 - sqrt(21197 - (8083.7*tar_vel));
  //u_c = (-0.03602 + sqrt(pow(0.03602,2)+(4*0.0001237*(-u_k))))/(-2*(0.0001237));
  /**/

 /*output command*/
  output = 2182.4 - sqrt((0.4*(2119700 - 808370*u_k)));
  //output = u_c * 4 + 1600 ;
  //output =   (u_c_target*4)+1600 ;//(u_c*4)
...
void CheckEN() {
  ...
  static float dis_err ,dis_P_err , dis_I_err , dis_target_vel ;
  static float dis_Kp_ = 4 ; // 1truck 5
  static float dis_Ki_ = 4.5 ; //2truck 6
  static float Ts = 0.1 ;
  static float TargetDist_;
  static float SafetyDist_ = 1.5;
  static float SafetySpeed_ = 0.3;
  static float TargetSpeed_ ;
  static float distance_;
  //target_RPM = (tx_throttle_ / circ_)* 60; // RPM
  
  //target_vel = tx_throttle_; // m/s
  TargetSpeed_ = tx_throttle_ ;
  TargetDist_ = tx_tdist_;
  distance_ = tx_dist_;
  ...
  {
    target_vel = (TargetSpeed_ - SafetySpeed_)*((distance_-TargetDist_)/(SafetyDist_-TargetDist_))
    +SafetySpeed_;
  }
  else{
    target_vel = tx_throttle_;
  }
   
  
  /* discrete PI*/
  //거리에 따라 앞,뒤차 속도를 제어하는 거니까 트럭간 거리를 v2v로 알아야 하는 것 아닌가 ? 
  dis_err = tx_dist_ - tx_tdist_ ; // p_i-1(k)  - p_i(k)  - l_i - d_r,i(k) << edit ?? 
  dis_P_err =  (dis_Kp_ * dis_err);
  dis_I_err += dis_Ki_ * dis_err * Ts ;
  dis_target_vel = dis_P_err + dis_I_err;
  ...
  //if(cur_vel == 0 && tar_vel !=0 ){
  //  cur_vel + 
  // }
  output_vel = setSPEED(target_vel, cur_vel);
  //output_vel = setSPEED(dis_target_vel, cur_vel);
