...
void set_params(LaneDetector* tmp, ros::NodeHandle nh) {
	...
	tmp->corners_[0] = Point2f(350, 450);
	tmp->corners_[1] = Point2f(930, 450);
	tmp->corners_[2] = Point2f(50, 680);
	tmp->corners_[3] = Point2f(1230, 680);
	tmp->wide_extra_upside_ = -50;
	tmp->wide_extra_downside_ = 50;
	tmp->warpCorners_[0] = Point2f(tmp->wide_extra_upside_, 0.0);
	tmp->warpCorners_[1] = Point2f(tmp->width_, tmp->wide_extra_upside_);
	tmp->warpCorners_[2] = Point2f(tmp->wide_extra_downside_, (float)tmp->height_);
	tmp->warpCorners_[3] = Point2f(tmp->width_ - tmp->wide_extra_downside_, (float)tmp->height_);
...
int main(int argc, char** argv) {
	...
	image_sub  = nh.subscribe("/usb_cam/image_raw", 1, ImageCallback);
	...
	if(!cap.isOpened())
		...
		while(ros::ok()){
			...
			cap >> frame_;
			if(!frame_.empty()) {
				msg.angular.z = lane_detect.display_img(frame_, delay);
				msg.linear.x = 1610;
				pub.publish(msg);
			}
		}
